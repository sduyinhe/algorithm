...
public class CollaborativeFilteringWithRecallTest {
    static Logger logger = LoggerFactory.getLogger(CollaborativeFilteringWithRecallTest.class);
...
//
//        model.write().overwrite().save("hdfs://localhost:9000/spark/userProductALSModel");

        ALSModel modelReadFromHDFS = ALSModel.load("hdfs://localhost:9000/spark/userProductALSModel");
// Evaluate the model by computing the RMSE on the test data
// Note we set cold start strategy to 'drop' to ensure we don't get NaN evaluation metrics
        // modelReadFromHDFS.transform(test).sort(desc("prediction")).na().drop().show();

        Dataset<Row> rowDataset = modelReadFromHDFS.recommendForAllUsers(10);
...
//        Row[] userDF = (Row[]) modelReadFromHDFS.userFactors().na().drop().select("id").collect();
//        for (int i = 0; i < userDF.length; i++) {
//            System.out.println((Integer)userDF[i].getAs("id"));
//        }
        Dataset<Row> productDF = modelReadFromHDFS.recommendForAllUsers(20);

        Encoder<RecommendationForUser> recommendationForUserEncoder = Encoders.bean(RecommendationForUser.class);
        Dataset<RecommendationForUser> df = productDF.as(recommendationForUserEncoder);
        df.collect();
        df.foreach(ele3 -> {
            Jedis jedis = RedisUtil.getJedis();
            //System.out.println(ele3);
            Integer userId = ele3.getUserId();
            //获取推荐产品集合
            List<String> productList1 = Arrays.asList(ele3.getRecommendations()).stream().map(ele2 -> ele2.getProductId().toString()).collect(Collectors.toList());
            String[] productArray = new String[productList1.size()];
            jedis.sadd("USER_ID_" + userId, productList1.toArray(productArray));
            RedisUtil.returnResource(jedis);
        });
        Jedis jedis = RedisUtil.getJedis();
        //取出用户10的集合
        Set<String> testProduct = jedis.smembers("USER_ID_10");
        // testProduct.forEach(System.out::println);
        //jedis.sadd()
        for (String productId : testProduct) {
            System.out.println("推荐产品ID为：" + productId + ",产品名称为：【" + productMap.get(productId) + "】");
        }
    }
    ...
    @Data
    public static class RecommendationForUser implements Serializable {
