...
import com.galaxy.abaoGenerator.Utility;
...
public class MyDataSourceProvider implements DataSourceProvider {
    ...
    private static BaseTableModel transformBaseTableModel(IntrospectedTable introspectedTable, GeneratorParam param) {
        ...
        for (IntrospectedColumn column : columnList) {
            ...
            columnInfo.setName(column.getActualColumnName());//实际列名
            columnInfo.setModelColumnName(Utility.underscoreToCamelCase(column.getActualColumnName()));//驼峰命名规则
            columnInfo.setComment(column.getRemarks());//注释
            javaType = column.getFullyQualifiedJavaType();
            columnInfo.setJavaType(javaType.getShortName());//java类型
        ...
        baseModel.setTableName(tableName);//表名
        baseModel.setTableComments(introspectedTable.getRemarks());
