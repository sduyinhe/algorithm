...
public class LispEvaluate
{
    ...
    {
        LispEvaluate lispEvaluate = new LispEvaluate();
        System.out.println(lispEvaluate.evaluate("(let a1 3 b2 (add a1 1) b2)"));
    }
    ...
    public int evaluate_inner(String expression)
    {
        ...
        //处理运算
        List<String> varList = parse(expression.substring(expression.charAt(1) == 'm' ? 6 : 5, expression.length() - 1));
        if (expression.startsWith("add", 1))
        {
            return evaluate(varList.get(0)) + evaluate(varList.get(1));
        ...
        {
            //let
            for (int i = 1; i < varList.size(); i += 2)
            {
                scope.get(scope.size() - 1).put(varList.get(i - 1), evaluate(varList.get(i)));
            }
            return evaluate(varList.get(varList.size() - 1));
        }
    }
